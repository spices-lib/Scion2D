...
namespace SCION_WINDOWING::Inputs {
	...
	void Keyboard::OnKeyPressed(int key)
	{
		if (key == KEY_UNKNOWN)
		{
			SCION_ERROR("Key {0} is unknown!", key);
			return;
		}

		if (!m_mapButtons.contains(key))
		{
			SCION_ERROR("Key {0} do not exist!", key);
			return;
		}

		m_mapButtons.at(key).Update(true);
	}
	...
	void Keyboard::OnKeyReleased(int key)
	{
		...
		m_mapButtons.at(key).Update(false);
	}
	...
	const bool Keyboard::IsKeyPressed(int key) const
	{
		...
		return m_mapButtons.at(key).bIsPressed;
	}
	...
	const bool Keyboard::IsKeyJustPressed(int key) const
	{
		...
		return m_mapButtons.at(key).bJustPressed;
	}
