...
namespace SCION_RENDERING {
				...
				for (const auto& character : textStr)
				{
					...
					if (currentFont == 0)
					{
						m_Batches.push_back(std::make_shared<TextBatch>(TextBatch{
							.offset = offset,
							.numVertices = 6,
							.fontAtlas = textGlyph->font->GetFontAtlasID()
						}));
					}
	...
	void TextBatchRenderer::Render()
	{
		...
		{
			glBindTexture(GL_TEXTURE_2D, batch->fontAtlas);
	...
	void TextBatchRenderer::AddText(const std::string& text, const std::shared_ptr<Font>& font, const glm::vec2& position, int padding, float wrap, Color color, glm::mat4 model)
	{
		...
		auto newTextGlyph = std::make_shared<TextGlyph>(TextGlyph{
			.textStr = text,
			.position = position,
			.color = color,
			.model = model,
		...
		m_Glyphs.push_back(newTextGlyph);
	}
}
